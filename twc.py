...
#train_sample = pd.read_csv(file_path1)
    train_sample=pd.read_csv(file_path1).sample(frac=0.01,replace=True, random_state=k, axis=0)
...
#
    ...
    for i in ('sgd','adam','lbfgs'):
        nn = MLPClassifier(solver=i, alpha=1e-5,hidden_layer_sizes=(6, 6), random_state=k+31)
